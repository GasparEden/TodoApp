...
const removeTodo: Function = (id: number) => {
    todos.splice(todos.indexOf(todos.find((i: object) => i['id'] == id)), 1);
    render();
}

const editTodo: Function = (elem: any, id: number) => {
    if(elem.checked) {
        todos.find(v => v['id'] == id)['done'] = true;
    } else {
        todos.find(v => v['id'] == id)['done'] = false;
    }
    render();
}

document.querySelector('#btn-add').addEventListener('click', (evt: Event) => {
    addTodo();
});
